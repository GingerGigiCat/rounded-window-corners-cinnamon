...
const ClipShadowEffect = require("./effect/clip_shadow_effect.js").ClipShadowEffect; //import { ClipShadowEffect } from '../effect/clip_shadow_effect.js';
const RoundedCornersEffect = require("./effect/rounded_corners_effect.js").RoundedCornersEffect; //import { RoundedCornersEffect } from '../effect/rounded_corners_effect.js';
//import { APP_SHADOWS, CLIP_SHADOW_EFFECT, ROUNDED_CORNERS_EFFECT, SHADOW_PADDING, } from '../utils/constants.js';
const APP_SHADOWS = require('./utils/constants.js').APP_SHADOWS;
const CLIP_SHADOW_EFFECT = require('./utils/constants.js').CLIP_SHADOW_EFFECT;
const ROUNDED_CORNERS_EFFECT = require('./utils/constants.js').ROUNDED_CORNERS_EFFECT;
...
const shouldEnableEffect = require("./utils/ui.js").shouldEnableEffect;
...
function onRestacked() {
    for (const actor of global.get_window_actors()) {
        const shadow = actor.__rwcRoundedWindowInfo
            ?.shadow;
...
const onSizeChanged = refreshRoundedCorners;
const onFocusChanged = refreshShadow;
 ...
 * */
function computeBounds(actor, [x, y, width, height]) {
    const bounds = {
        x1: x + 1,
        y1: y + 1,
 ...
 */
function computeShadowActorOffset(actor, [offsetX, offsetY, offsetWidth, offsetHeight]) {
    const win = actor.metaWindow;
 ...
 * */
function updateShadowActorStyle(win, actor, borderRadiusRaw = settings().global_rounded_corner_settings.border_radius, shadow = settings().focused_shadow, padding = settings().global_rounded_corner_settings.padding) {
